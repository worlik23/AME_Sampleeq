...
if(!empty($_POST['with'])){
    $with = implode(', ', $_POST['with']);
    $rqParams = "AND status IN($with)";
    $_SESSION['with'] = $_POST['with'];
} else {	$withs = ['1', '2'];
        	$with = implode(', ', $withs);
        	$rqParams = "AND status IN($with)";
        	$_SESSION['with'] = $withs;
...
<h1>Search</h1>
<div id="searchForms">
<form class="getProject" method="post" onsubmit="saveDepartment()">
...
<select id="hub" name="hub">
    ...
    <option value="B">Bluèina</option>
</select>
<?php echo (isMobile()) ? '</div>' : ''; ?>
<div class="row">
<?php 
    if($userOn && $adminLevel !== 'tech' && $adminLevel !== 'sap'){
...
<hr>
<h2>Search by SN#</h2>
<form method="post" class="getProject">
...
else if((!isset($_POST['getInfo']) || !isset($_POST['search'])) && !empty($_SESSION['searchRq'])){
